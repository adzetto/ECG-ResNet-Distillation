...
class ResBlock(torch.nn.Module):
    def __init__(self, in_ch, out_ch, stride=1):
        super().__init__()
        self.conv1 = torch.nn.Conv1d(in_ch, out_ch, kernel_size=7, stride=stride, padding=3)
        self.bn1 = torch.nn.BatchNorm1d(out_ch)
        self.conv2 = torch.nn.Conv1d(out_ch, out_ch, kernel_size=7, stride=1, padding=3)
...
def normalize_labels(label_names):
    normalized = []
...
def load_model(path, device):
    ...
    label_names = normalize_labels(checkpoint.get("label_names") or [])
    num_classes = len(label_names) if label_names else 2
    model_name = checkpoint.get("model_name", "basic")
    in_channels = int(checkpoint.get("in_channels", 1))
    if in_channels != 1:
        raise SystemExit(
            f"Model {path} expects {in_channels} channels; predict_twostage expects single-lead."
        )
...
def label_index(label_names, target, fallback):
    upper = [name.upper() for name in label_names]
    if target in upper:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Two-stage ECG inference")
    parser.add_argument("--stage1", required=True, help="Stage1 model path")
    parser.add_argument("--stage2", required=True, help="Stage2 model path")
...
def main():
    ...
    stage1_model, stage1_labels = load_model(args.stage1, device)
    stage2_model, stage2_labels = load_model(args.stage2, device)
    ...
    with torch.no_grad():
        for _, _, segment in iter_windows(resampled, window_size, stride):
            ...
            stage1_logits = stage1_model(x)
            stage1_pred = int(torch.argmax(stage1_logits, dim=1).cpu().item())
            stage1_preds.append(stage1_pred)

            stage2_logits = stage2_model(x)
            stage2_pred = int(torch.argmax(stage2_logits, dim=1).cpu().item())
    ...
    arr_idx = label_index(stage1_labels, "ARRHYTHMIA", 1)
    arr_votes = sum(1 for p in stage1_preds if p == arr_idx)
    stage1_vote = "ARRHYTHMIA" if arr_votes > (len(stage1_preds) // 2) else "SINUS"

    if stage1_vote == "ARRHYTHMIA":
        final_label = "ARRHYTHMIA"
    else:
        mi_idx = label_index(stage2_labels, "MI", 1)
        mi_votes = sum(1 for p in stage2_preds if p == mi_idx)
        final_label = "MI" if mi_votes > (len(stage2_preds) // 2) else "NORMAL"

    print(f"Stage1 vote: {stage1_vote} ({arr_votes}/{len(stage1_preds)} arrhythmia windows)")
